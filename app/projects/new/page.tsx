            ...
            <form onSubmit={createProject} className="min-h-screen">
              ...
              ═══════════════════════════════════════════════════════════════ */}
                        ...
                        <div className="lg:col-span-4 xl:col-span-3 flex flex-col lg:h-[520px] gap-2">
                            ...
                            <div className="rounded-xl bg-white border border-slate-200 overflow-hidden flex flex-col flex-1 min-h-0 shadow-sm">
                                <div className="px-3 py-2 border-b border-slate-100 flex items-center justify-between shrink-0">
                                    <p className="text-xs font-semibold text-slate-900">{t("newProj.parcels")}</p>
                                    {selectedAddress && !loadingCadastre && parcels.length > 0 && (
                                        <span className="text-[10px] text-slate-500">{parcels.length} {t("newProj.found")}</span>
                                    )}
                                </div>
                                                ...
                                                {parcels.map((p) => {
                                                    ...
                                                    return (
                                                        <div key={p.id} role="button" tabIndex={0} aria-pressed={selected}
                                                            ...
                                                            className={cn(
                                                                "flex items-center gap-1.5 px-2 py-1.5 rounded-lg text-[11px] transition-all cursor-pointer border",
                                ...
                                {selectedParcelIds.length > 0 && (
                                    <div className="px-3 py-2 border-t border-slate-100 flex items-center justify-between shrink-0 bg-slate-50">
                                        <span className="text-[10px] text-slate-500">{selectedParcelIds.length} {t("newProj.selected")}{selectedParcelIds.length > 1 ? "s" : ""}</span>
                                        <span className="text-sm font-bold text-emerald-600 tabular-nums">{totalSelectedArea.toLocaleString()} m²</span>
                                    </div>
                                )}
                            </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 shrink-0">
                                ...
                                <div className="rounded-xl bg-white border border-slate-200 overflow-hidden shadow-sm">
                                    <div className="px-3 py-2 border-b border-slate-100 flex items-center gap-1.5">
                                        ...
                                        <p className="text-xs font-semibold text-slate-900">{t("newProj.zonePlu")}</p>
                                    </div>
                                    <div className="p-2.5">
                                ...
                                <div className="rounded-xl bg-white border border-slate-200 overflow-hidden flex flex-col shadow-sm">
                                    <div className="px-3 py-2 border-b border-slate-100 flex items-center gap-1.5 shrink-0">
                                        ...
                                        <p className="text-xs font-semibold text-slate-900 flex-1 truncate">{t("newProj.protections")}</p>
                                                ...
                                                {protectedAreas.map((area: { type: string; name: string; severity?: string; sourceUrl?: string | null; description?: string }, idx: number) => {
                                                    ...
                                                    return (
                                                        <div key={idx} className="flex items-center gap-1.5 py-1 px-1.5 rounded-lg hover:bg-slate-50 transition-colors">
                                                            ...
                                                            <p className="text-[10px] text-slate-600 truncate flex-1">{area.name}</p>
                    ...
                    <div className="flex items-center gap-3 py-3 border-t border-slate-200 sticky bottom-0 bg-white/90 backdrop-blur-lg -mx-4 px-4 lg:-mx-6 lg:px-6">
                        ...
                        >
                            {creating ? <span className="flex items-center gap-2"><Loader2 className="w-4 h-4 animate-spin" /> {t("newProj.creating")}</span> : t("newProj.createProject")}
                        </button>
                    </div>
                </div>
            </form>
