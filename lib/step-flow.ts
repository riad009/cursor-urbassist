...
export const PLANNING_STEPS = [
  ...
  { step: 2, label: "PLU Analysis", path: "/plu-analysis", pathMatch: (p: string) => p === "/plu-analysis" || p.startsWith("/plu-analysis/"), href: (id: string) => `/plu-analysis?project=${id}` },
  { step: 3, label: "Location Plan", path: "/location-plan", pathMatch: (p: string) => p === "/location-plan" || p.startsWith("/location-plan/"), href: (id: string) => `/location-plan?project=${id}` },
  { step: 4, label: "Site Plan", path: "/site-plan", pathMatch: (p: string) => p === "/site-plan" || p.startsWith("/site-plan/") || p === "/editor" || p.startsWith("/editor/"), href: (id: string) => `/site-plan?project=${id}` },
  { step: 5, label: "Terrain", path: "/terrain", pathMatch: (p: string) => p === "/terrain" || p.startsWith("/terrain/"), href: (id: string) => `/terrain?project=${id}` },
  { step: 6, label: "Building 3D", path: "/building-3d", pathMatch: (p: string) => p === "/building-3d" || p.startsWith("/building-3d/"), href: (id: string) => `/building-3d?project=${id}` },
  { step: 7, label: "Landscape", path: "/landscape", pathMatch: (p: string) => p === "/landscape" || p.startsWith("/landscape/"), href: (id: string) => `/landscape?project=${id}` },
